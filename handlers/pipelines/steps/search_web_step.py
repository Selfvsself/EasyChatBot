...
class SearchWebStep(PipelineStep):
    ...
    async def execute(self, context: PipelineContext, stage_callback=None) -> StepResult:
        ...
        search_results = []
        seen_urls = set()
        search_queries = self.get_search_queries(context)
        for s_query in search_queries:
            await self._emit_stage(stage_callback, stage="web_search_running", metadata={"query": s_query})
            web_results = await self.web_search_tool.search(s_query)
